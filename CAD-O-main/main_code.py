...
def Module_1(gpkg_filepath,XYZfile, GEOADMIN_BASE_URL,
    ...
    MO_dhn = gpd.read_file(gpkg_filepath, layer = "zone_cad_new")
    ...
    # road_index_list, green_index_list, buffered_streets = xml.modify_type(district, ground_data, green_data, street_data)
    road_index_list, buffered_streets = xml.scenario1(district, ground_data, street_data)
    


    # TS_file = r"D:\Document\SemesterProject\New_case\VD\TS_no_ground_under_building\morges_default_TS.tsv"
    # TS_df = pd.read_csv(TS_file, delimiter='\t')
    # grounds_AST, buildings_AST, all_AST = xml.AST(envelope, TS_df, ground_data)
    # buildings_AST.to_file(gpkg_filepath, layer='buildings_AST_s1', driver='GPKG')
    # grounds_AST.to_file(gpkg_filepath, layer='grounds_AST_s1', driver='GPKG')
    # all_AST.to_file(gpkg_filepath, layer='all_AST_s1', driver='GPKG')



    #approximation comparison
    print('approximation comparison \n') 
    # selected_grounds1=ground_data.iloc[road_index_list]
    # selected_grounds2=ground_data.iloc[green_index_list]
    # # add layers of green areas and streets
    # selected_grounds1.to_file(gpkg_filepath, layer='street_grounds_fine', driver='GPKG')
    # selected_grounds2.to_file(gpkg_filepath, layer='green_grounds_fine', driver='GPKG')

    # fig1, ax1 = plt.subplots()
    # buffered_streets.geometry.plot(ax=ax1, color='blue', label='original')
    # selected_grounds1.geometry.plot(ax=ax1, color='red', alpha=0.5, label='approx')
    # ax1.set_xlabel('Longitude')
    # ax1.set_ylabel('Latitude')
    # ax1.set_title('streets')
    # ax1.legend()
    # fig1.savefig(r'D:\Document\SemesterProject\New_case\VD\roads.png')

    # fig2, ax2 = plt.subplots()
    # green_data.geometry.plot(ax=ax2, color='blue', label='original')
    # selected_grounds2.geometry.plot(ax=ax2, color='red', alpha=0.5, label='approx')
    # ax2.set_xlabel('Longitude')
    # ax2.set_ylabel('Latitude')
    # ax2.set_title('green_areas')
    # ax2.legend()
    # fig2.savefig(r'D:\Document\SemesterProject\New_case\VD\green_areas.png')

    #write xml file
    print('creating xml file \n')
    # xml_name37=xml_name+"_soil"
    # xml_name2=xml_name+"_road"
    # xml_name3=xml_name+"_green"

    # xml.keep_soil(district)
    # xml_path37 = os.path.join(directory_path, xml_name37+".xml")     
    # xml.write_xml_file(root, xml_path37) 

    # xml.keep_road(district)
    # xml_path2 = os.path.join(directory_path, xml_name2+".xml")     
    # xml.write_xml_file(root, xml_path2) 

    # xml.keep_green(district)
    # xml_path3 = os.path.join(directory_path, xml_name3+".xml")     
    # xml.write_xml_file(root, xml_path3) 
   
   # simplify for citysim
...
def Module_2(directory_path, xml_name, zone_dhn, centrale,
    ...
    for i in range(len(scenarios)):
        ...
        if sc_id in scenarios_list:
            ...
            # Efficiency data
            technology_parameters = pd.read_csv("D:\Document\SemesterProject\CAD-O-main\KPI.csv", delimiter=",")
...
# Geopackage filepath
gpkg_filepath = r"D:\Document\SemesterProject\New_case\VD\study_area\morges.gpkg"                               #TODO
...
# XML name to export
directory_path = r"morges_result_scenario1"        #cut2                           #TODO
...
# XML source files
xml_base_file = r"D:\Document\SemesterProject\CAD-O-main\xml_base.xml"                               
climate_file = r"D:\Document\SemesterProject\New_case\VD\cli\Morges_Contemporary.cli"                                   #TODO
horizon_file = r"D:\Document\SemesterProject\Bex.hor"                                        #TODO
XYZfile = r"D:\Document\SemesterProject\New_case\VD\study_area\SWISSALTI3D_2_XYZ_CHLV95_LN02_2527_1151.xyz" #TODO
XYZfile_fine =r"D:\Document\SemesterProject\New_case\VD\study_area\SWISSALTI3D_0.5_XYZ_CHLV95_LN02_2527_1151.xyz"
# Scenarios to simulate
#scenarios_list = [1,2,3,4,5,6]  
...
def main(): 
    ...
    TS_df = pd.read_csv(TS_file, delimiter='\t')
    grounds_AST, buildings_AST, all_AST = xml.AST(envelope, TS_df, ground_data)
    # buildings_AST.to_file(gpkg_filepath, layer='buildings_AST_dft', driver='GPKG')
    # grounds_AST.to_file(gpkg_filepath, layer='grounds_AST_dft', driver='GPKG')
    all_AST.to_file(gpkg_filepath, layer='all_AST_s1', driver='GPKG')
